                                    ...
                                    <table id="datatable" class="table  table-hover table-sm table-bordered p-0"
                                        ...
                                        @foreach($books as $book)
                                            <tr>
                                                ...
                                                <td>{{$book->title}}</td>
                                                <td>{{$book->teacher->First_name. "  " .$book->teacher->Second_name." ".$book->teacher->Last_name}}</td>
                                                <td>{{$book->grade->name}}</td>
                                                <td>{{$book->section->Name_Section}}</td>
                                                <td>
                                                    <a href="{{route('downloadAttachment',$book->file_name)}}" title="تحميل الكتاب" class="btn btn-warning btn-sm" role="button" aria-pressed="true"><i class="fa fa-cloud-download" aria-hidden="true"></i></i></a>
